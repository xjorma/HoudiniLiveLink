...
// DGallardo Modified Sample to sent coordinate througth the Network
...
{
public:
    virtual void EmitFrame(const std::string &str) = 0;
    virtual ~PositionStreamer() = 0 {}
};

// https://social.msdn.microsoft.com/Forums/en-US/f7402dfa-405c-4b6d-95d8-3d10530d8120/using-server-and-a-client-with-udp

class UDPStreamer : public PositionStreamer
{
    ...
    sockaddr_in dest;
public:
    UDPStreamer(const char* ipAdress, int port)
    {
        InitWinsock();
        // Create Socket
    ...
    void EmitFrame(const std::string& str) override
    {
        sendto(socketS, str.c_str(), static_cast<int>(str.length()), 0, (sockaddr*)&dest, sizeof(dest));
    }
    ~UDPStreamer() override
    {
        closesocket(socketS);
    }
};

class FileStreamer : public PositionStreamer
{
    std::ofstream outFile;
public:
    FileStreamer(const char* fileName)
    {
...
void SendBones(k4abt_body_t & body)
{
    std::stringstream ss;
    for (int joint = 0; joint < static_cast<int>(K4ABT_JOINT_COUNT); joint++)
    {
        ...
        k4abt_joint_id_t joinId = static_cast<k4abt_joint_id_t>(joint);
        ss << g_jointNames.find(joinId)->second << "|" << jointPosition.v[0] << "|" << jointPosition.v[1] << "|" << jointPosition.v[2] << "|" << jointOrientation.wxyz.x << "|" << jointOrientation.wxyz.y << "|" << jointOrientation.wxyz.z << "|" << jointOrientation.wxyz.w;
    }
    ss << std::endl;
    posStream->EmitFrame(ss.str());
}
...
int main(int argc, char** argv)
{
    ...
    if (ParseInputSettingsFromArg(argc, argv, inputSettings))
    {
        ...
        {
            posStream = new FileStreamer("d://kinect6.txt");
        }
